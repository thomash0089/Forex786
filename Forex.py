# --- Forex AI Signals (15-Min | Improved Accuracy & Confirmation) ---
...
def detect_divergence_direction(df):
    ...
    if len(lows) >= 2 and close.iloc[lows[-1]] < close.iloc[lows[-2]] and rsi.iloc[lows[-1]] > rsi.iloc[lows[-2]]:
        return "Bullish"
    if len(highs) >= 2 and close.iloc[highs[-1]] > close.iloc[highs[-2]] and rsi.iloc[highs[-1]] < rsi.iloc[highs[-2]]:
        return "Bearish"
    return ""
...
def generate_ai_suggestion(price, direction, indicators):
...
for label, symbol in symbols.items():
    ...
    if df is not None:
        ...
        news_block = check_news_alert(label)
        ...
        if direction:
            ...
            if direction in detect_candle_pattern(df):
                indicators.append("Candle")

        ai_suggestion = ""
        if direction == trend and len(indicators) >= 3 and not news_block:
            ai_suggestion = generate_ai_suggestion(price_now, direction, indicators)

        rows.append({
            "Pair": label, "Price": round(price_now, 5), "RSI": round(df['RSI'].iloc[-1], 2),
            "Trend": trend, "Divergence": direction, "TF Confirm": tf_status,
            "Confirmed Indicators": ", ".join(indicators), "AI Suggestion": ai_suggestion
        })

# --- Display ---
st.title("📊 Forex AI Signals (15-Min Accuracy Enhanced)")
df_final = pd.DataFrame(rows)
st.dataframe(df_final.style.applymap(
    lambda val: 'background-color: #d1ffd1' if isinstance(val, str) and 'Strong' in val else ''
